...
import { HostMonitor, type HostMachineSummary } from './hostMonitor';
...
async function run(): Promise<void> {
  ...
  core.info(`machine-cpu-threshold:    ${inputs.machineCpuThreshold}% (machine-wide)`);
  core.info(`machine-memory-threshold: ${inputs.machineMemoryThreshold}% of RAM limit`);
  ...
  // ---- Layer 1 (live): machine-wide sampling inside the action ----------
  // Linux-only /proc sampler. Used as the machine layer of record unless a
  // monocart report exists (then monocart wins, documented precedence), but
  // always provides live breach alerts with zero user-side setup.
  ...
  // ---- Layer 1: machine-wide evaluation ---------------------------------
  // Source of record: a monocart report when present, otherwise the
  // action's own /proc sampler.
  const monocartPath = resolveWorkPath(inputs.monocartJson);
  const { reportFound: monocartFound, readings } = loadReadings(monocartPath);
  const hostSummary: HostMachineSummary = hostMonitor
    ? hostMonitor.stop()
    : { samples: [], breaches: [], peakCpuPercent: 0, peakMemoryPercent: 0, peakMemoryBytes: 0, source: null };
  const usingMonocart = monocartFound && readings.length > 0;
  const machine = usingMonocart
    ? evaluateMachine(readings, {
        cpuThreshold: inputs.machineCpuThreshold,
        memoryThreshold: inputs.machineMemoryThreshold,
      })
    : {
  ...
  const machineSource: 'monocart' | 'action' | null = usingMonocart
    ? 'monocart'
    : hostSummary.source;
  if (usingMonocart && hostSummary.samples.length > 0) {
    core.debug(
      'monocart report found — machine metrics sourced from monocart ticks; action /proc live alerts remain advisory.',
    );
  }
  ...
  const entry: HistoryEntry = {
    ...
    machine: machineSource
      ? {
          peakCpuPercent: usingMonocart
            ? machine.peakCpuPercent
            : hostSummary.peakCpuPercent,
          peakMemoryPercent: usingMonocart
            ? machine.peakMemoryPercent
            : hostSummary.peakMemoryPercent,
  ...
  if (!machineSource && tabSummary.readings.length === 0) {
    // Perfectly fine: the wrapped command launched no browsers, the platform
    // offers no machine sampler (non-Linux), and there is no monocart
    // report. The action simply passes through.
    core.info(
      'No Playwright test activity detected (no browser processes, no machine sampler, no monocart report) — ' +
        'resource monitoring skipped; nothing was enforced. This is not an error.',
    );
  } else if (!machineSource) {
    core.info(
      'Machine-wide layer unavailable on this platform (the built-in sampler is Linux-only; ' +
        'configure monocart-reporter for machine thresholds) — the per-tab layer was active.',
    );
  } else if (!monocartFound) {
    core.info(
      'Machine layer used the built-in Linux /proc sampler (no monocart report found — ' +
        'add monocart-reporter with json:true for an HTML timeline); per-tab layer active.',
    );
  } else {
    core.info(
...
function buildBreachMessage(
  ...
  machineSource: 'monocart' | 'action' | null,
  ...
  if (machineBreached) {
    parts.push(
      ...
      `machine memory at ${machine.peakMemoryPercent.toFixed(1)}% of RAM (threshold ${inputs.machineMemoryThreshold}%) ` +
...
function setOutputs(
  ...
  core.setOutput('machine-source', machineSource ?? 'none');
  ...
  core.setOutput('monocart-found', String(machineSource === 'monocart'));
